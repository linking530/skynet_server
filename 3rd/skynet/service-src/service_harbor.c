...
struct harbor_msg_queue {
	...
	int head;
	int tail;
	struct harbor_msg * data;// data[] slot保存了具体的数据而不是指针 这里的实现为固定大小的环形缓冲区 大小不够时是2倍的扩张
};
...
push_queue_msg(struct harbor_msg_queue * queue, struct harbor_msg * m) {
	...
	struct harbor_msg * slot = &queue->data[queue->tail]; // 找到这个队列的slot放入这个msg
	*slot = *m;
...
new_queue() {
	...
	queue->head = 0;
	queue->tail = 0;
	queue->data = skynet_malloc(DEFAULT_QUEUE_SIZE * sizeof(struct harbor_msg));// 默认队列大小为1024的环形队列
...
hash_insert(struct hashmap * hash, const char name[GLOBALNAME_LENGTH]) {
	...
	struct keyvalue * node = skynet_malloc(sizeof(*node));     // 新分配一个 node 出来 防止 name
...
harbor_init(struct harbor *h, struct skynet_context *ctx, const char * args) {
	...
	int harbor_id = 0;
	uint32_t slave = 0;
	sscanf(args,"%d %u", &harbor_id, &slave);// sscanf()将args里面的数据格式化输出到这3个值中
	if (slave == 0) {
		return 1;
	}
	h->id = harbor_id;
