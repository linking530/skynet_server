...
struct socket_message {
	...
	int ud;	// for accept, ud is new connection id ; for data, ud is size of data  对于accept连接来说, ud是新连接的id;对于数据（data）来说, ud是数据的大小  
	char * data;// 对于数据收发事件，存放数据；对于socket连接事件，存放地址；其他情况下还可以存放打印日志。
};
