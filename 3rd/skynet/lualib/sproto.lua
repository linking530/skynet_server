...
--
--The sproto supports protocol tag for RPC.
--Use `sproto.protocol(tagorname)` to convert the protocol name to the tag id,
-- or convert back from tag id to the name,
...
local function queryproto(self, pname)
	...
	if not v then
--`sproto.protocol(tagorname)` to convert the protocol name to the tag id,	
