...
local packvalue
--
--f'x' 在Lua里等价于 f('x') ; 单引号和双引号等价)
-- string.pack (fmt, v1, v2, ···)
--<: 设为小端编码
--s[n]: 长度加内容的字符串，其长度编码为一个 n 字节（默认是个 size_t） 长的无符号整数。
--I一个 n 字节长（默认为本地大小）的无符号 int
--
...
---等价于lpeg.P(1) 
-- As a not so simple example, -lpeg.P(1) (which can be written as lpeg.P(-1), 
-- or simply -1 for operations expecting a pattern) matches an empty string only 
-- if it cannot match a single character; so, it succeeds only at the end of the subject. 
local eof = P(-1)
local newline = lpeg.Cmt((P"\n" + "\r\n") * lpeg.Carg(1) ,count_lines)
--注释
--patt^0 is equivalent to patt*
