...
-- 可以对一个数据块进行解包工作。unpack 必须提供数据块以及相应的 pattern 对象。
-- 它会按 pattern 的指示把结构化数据解到栈上。
-- 这种方式回避了每次解包都生成临时的 table 。
-- 当然，如果使用者还是喜欢构造一个 table 来表达数据块中的数据结构的话，
-- 依然可以通过几行 lua 封装代码来办到。
