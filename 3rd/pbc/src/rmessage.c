...
// 由于在编码后每一个字段的key都是varint类型，key的值是由字段标号和字段类型合成编码所得，
// 其公式如下：field_number << 3 | field_type
struct pbc_rmessage {
		struct _message * msg;	//struct _message * type
...
//当你能肯定你需要的整数可以用 32bit 描述时，
//pbc_rmessage_integer 的最后一个参数可以传 NULL ，忽略高 32bit 的数据。
uint32_t
