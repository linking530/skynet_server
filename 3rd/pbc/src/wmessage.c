...
struct pbc_wmessage {
	//struct _message * msg = _pbcP_get_message(env, "tutorial.Person");
	//m->type = msg;
	struct _message *type;	//模板
	//-------参考_expand_message函数----------------------
	uint8_t * buffer;	//内存分配的位置
	uint8_t * ptr;		//当前位置，已经使用到的位置
	uint8_t * endptr;	//分配的内存结束位置 确定结构的开始结束位置 m->endptr = m->buffer + WMESSAGE_SIZE;
	//----------------------------------------------
	pbc_array sub;
	struct map_sp *packed;
	struct heap * heap;	//结构所属内存分配器
};
...
//确定m剩余的缓冲区长度大于sz，如果小于sz则增加内存空间给m，新分配的cp为2的整数倍
static void
...
//获得key相对应的包，如果没有则用f和struct pbc_wmessage *m生成一个并将static struct _packed *p的heap指针和m的heap指针相关联
...
_pack_packed_varint(struct _packed *p,struct pbc_wmessage *m) {
	...
	default:
		...
		memset(m->ptr , 0 , n);
		m->ptr += n;
		//将错误信息写入 环境指针，环境指针挂在m上
		m->type->env->lasterror = "wmessage type error when pack packed";
		break;
	}
	int end_offset = m->ptr - m->buffer;//已经写入的数据偏移量
	int end_len = end_offset - (offset + len_len);//switch (p->ptype) {。。。。}中写入的字节
	//感觉这里是对前面全部按float_encode(var->real , m->ptr + i * 8);位移和其实只用了4字节的修正
	if (end_len != len) {
		uint8_t temp[10];
