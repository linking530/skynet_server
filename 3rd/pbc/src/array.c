...
/*初始化pbc_array _array,将heap设置给_array*/
void 
...
_pbcA_push(pbc_array _array, pbc_var var) {
	...
	if (a->number == 0) {
		...
		if (a->number == INNER_FIELD) {//如果元素等于INNER_FIELD的个数
			int cap = 1;
			while (cap <= a->number + 1) //增加a->number + 1或a->number + 1的倍数的空间
				cap *= 2;
...
//取出 _array数组 idx位置的数据放入var[0]位置
void 
