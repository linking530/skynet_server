...
//每个环境需要独立注册需要的消息类型，传入一个 protobuf 库官方工具生成的 .pb 数据块即可。
//以 slice 的形式传入，register 完后，这块数据内存可以释放。
//这个数据块其实是以 google.protobuf.FileDescriptorSet 类型来编码的。
//这个数据类型非常繁杂，使得 bootstrap 过程及其难写，这个在后面会谈到。
static const char *
...
pbc_register(struct pbc_env * p, struct pbc_slice *slice) {
	...
	//const char * key = NULL;
	//struct pbc_env * env = p;
	//printf("env.files:===========================================\n");

	//for (;;) {
	//	void * v = _pbcM_sp_next((struct map_sp *)env->files, &key);
	//	if (key == NULL)
	//		break;
	//	printf("%s : %s\n", key, (const char *)v);
	//}
	//printf("env.enums:===========================================\n");
	//for (;;) {
	//	void * v = _pbcM_sp_next((struct map_sp *)env->enums, &key);
	//	if (key == NULL)
	//		break;
	//	printf("%s : %s\n", key, (const char *)v);
	//}
	//printf("env.msgs:===========================================\n");
	//for (;;) {
	//	void * v = _pbcM_sp_next((struct map_sp *)env->msgs, &key);
	//	if (key == NULL)
	//		break;
	//	printf("%s : %s\n", key, (const char *)v);
	//}
