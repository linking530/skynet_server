...
// 更重要的是，对于比较小的消息包，如果你使用得当，
// 使用 pattern api 甚至不会触发哪怕一次堆上的内存分配操作。
// api 工作时的所有的临时内存都在栈上。
static void
...
//pbc_pattern_set_default 可以把一块内存，以一个 pattern 的形式，初始化所有的域。
//包括其中的数组的初始化。
void
...
_string:
	...
	return ret;
_number:
	len = _pack_wiretype(pf->id << 3 | wiretype , s);//Key的计算方式 Key = field_number << 3 | Wire_Type
	if (len < 0) {
		return len;
	}
	ret += len;
